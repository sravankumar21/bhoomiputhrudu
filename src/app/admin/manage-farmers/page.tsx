    ...
    <div className="relative min-h-screen bg-bg overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16">
        <div ref={headerRef} className="scroll-hidden flex items-center gap-4 mb-10">
          ...
          <div>
            <p className="text-xs font-medium uppercase tracking-[0.2em] text-primary">Farmers</p>
            <h1 className="font-[family-name:var(--font-playfair)] text-2xl md:text-3xl font-semibold text-text">{t.admin.manageFarmers}</h1>
          </div>
        </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-primary-50 text-left">
                  <tr>
                    <th className="px-6 py-4 font-semibold text-text-muted text-xs uppercase tracking-wider">{t.admin.name}</th>
                    <th className="px-6 py-4 font-semibold text-text-muted text-xs uppercase tracking-wider">{t.admin.mobileNumber}</th>
                    <th className="px-6 py-4 font-semibold text-text-muted text-xs uppercase tracking-wider">{t.admin.email}</th>
                    <th className="px-6 py-4 font-semibold text-text-muted text-xs uppercase tracking-wider">{t.admin.registeredOn}</th>
                ...
                <tbody className="divide-y divide-border">
                  {filtered.map((farmer) => (
                    <tr key={farmer._id} data-scroll-child className="hover:bg-primary-50/50 transition-colors duration-300">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 bg-gradient-to-br from-primary to-primary-light text-white rounded-xl flex items-center justify-center font-bold text-sm shadow-md">
                      ...
                      <td className="px-6 py-4 text-text-muted">{farmer.mobile_no}</td>
                      <td className="px-6 py-4 text-text-muted">{farmer.email || "—"}</td>
