    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center gap-2 mb-2 text-sm text-gray-500">
          <Link href="/products" className="hover:text-green-600">{t.allProducts}</Link>
          <span>/</span>
          <Link href={`/products/${category}`} className="hover:text-green-600">{t[category] || category}</Link>
          <span>/</span>
          <span className="text-gray-800 font-medium">{subcategory}</span>
        </div>
        <div className="flex items-center gap-4 mb-8">
          <Link href={`/products/${category}`} className="text-green-600 hover:text-green-700">
            <ArrowLeft className="w-5 h-5" />
          </Link>
          <h1 className="text-2xl md:text-3xl font-bold text-gray-800">{subcategory}</h1>
        </div>
